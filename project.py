...
class RGBImage:
    ...
    def __init__(self, pixels):
        ...
        if not all([isinstance(i,list) for i in pixels]):
            raise TypeError
        len_of_one = len(pixels[0])
        if not all([len(i)== len_of_one for i in pixels]):
            raise TypeError
        if len(pixels[0])==0:
            raise TypeError
        if not all([isinstance(j,list) for i in pixels for j in i]):
            raise TypeError
        if not all([isinstance(c, int) and c>=0 and c<=255 for a in pixels for b in a for c in b]):
            raise ValueError
        self.pixels = pixels
    ...
    def get_pixels(self):
        ...
        return [[[c for c in b]for b in a] for a in self.pixels]
    ...
    def copy(self):
        ...
        """
        # YOUR CODE GOES HERE #
        new_ob = RGBImage(self.get_pixels())
        return new_ob
    ...
    def get_pixel(self, row, col):
        ...
        if row>=self.num_rows or col>=self.num_cols:
            raise ValueError
        t_pixel = tuple(self.pixels[row][col])
        return t_pixel
    ...
    def set_pixel(self, row, col, new_color):
        ...
        if not isinstance(row, int) or not isinstance(col,int):
            raise TypeError
        if row<0 or col<0 or row>len(self.pixels) or col>len(self.pixels[0]):
            raise ValueError()
...
class ImageProcessingTemplate:
    ...
    def negate(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        return RGBImage([[[255-c for c in b]for b in a] for a in image.get_pixels()])
    ...
    def grayscale(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        average = lambda x: (x[0]+x[1]+x[2])/3
        #gray = map(list(map(average,image)),image )
        gray = [[[int((x[0]+x[1]+x[2])/3) for sing in x] for x in dub_list] for dub_list in image.get_pixels()]
    ...
    def rotate_180(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        f_pixels = image.get_pixels()[::-1]
        l_pixels = [dub_list[::-1] for dub_list in f_pixels]
        return RGBImage(l_pixels)
    ...
    def get_average_brightness(self, image):
        ...
        """
        # YOUR CODE GOES HERE #
        averages = [int((x[0]+x[1]+x[2])//3) for dub_list in image.get_pixels() for x in dub_list]
        total_pixels = len(image.get_pixels())*len(image.get_pixels()[0])
    ...
    def adjust_brightness(self, image, intensity):
        ...
        if type(intensity)!=float:
            return TypeError
        brighter= [[[int(intensity*sing) if int(intensity*sing)<=255 else 255 for sing in x] for x in dub_list] for dub_list in image.get_pixels()]
...
class PremiumImageProcessing(ImageProcessingTemplate):
    ...
    def pixelate(self, image, block_dim):
        ...
        """
        # YOUR CODE GOES HERE #
        final_list= image.get_pixels()
        cur_list = image.get_pixels()
        for row_int in range(0,len(cur_list),block_dim):
            for col_int in range(0, len(cur_list), block_dim):
                r_avg=0
                g_avg=0
                b_avg=0
                for i in range(block_dim):
                    for j in range(block_dim):
                        r_avg+= cur_list[row_int+i][col_int+j][0]
                        g_avg+= cur_list[row_int+i][col_int+j][1]
                        b_avg += cur_list[row_int+i][col_int+j][2]
                r_avg = r_avg//(block_dim**2)
                g_avg = g_avg//(block_dim**2)
                b_avg= b_avg//(block_dim**2)
                for i in range(block_dim):
                    for j in range(block_dim):
                        final_list[row_int+i][col_int+j][0]=r_avg
                        final_list[row_int+i][col_int+j][1]= g_avg
                        final_list[row_int+i][col_int+j][2]= b_avg
        return RGBImage(final_list)
    ...
    def edge_highlight(self, image):
        ...
        for row in range(len(pixels)):
            for col in range(len(pixels[0])):
                avg= (pixels[row][col][0] +pixels[row][col][1] + pixels[row][col][2])//3
                pixels[row][col] = avg
        n_pixels = [[j for j in i ] for i in pixels]
        t_sum=0

        kernel = [[-1,-1,-1], [-1,8,-1], [-1,-1,-1]]
        for row in range(len(pixels)):
            for col in range(len(pixels[0])):
                t_sum=8*pixels[row][col]
                if row-1 in range(len(pixels)):
                    t_sum-=pixels[row-1][col]
                if col-1 in range(len(pixels[0])):
                    t_sum -=pixels[row][col-1]
                if col+1 in range(len(pixels[0])):
                    t_sum -=pixels[row][col+1]
                if row+1 in range(len(pixels)):
                    t_sum-=pixels[row+1][col]
                if row-1 in range(len(pixels)) and col+1 in range(len(pixels[0])):
                    t_sum-=pixels[row-1][col+1]
                if row+1 in range(len(pixels)) and col+1 in range(len(pixels[0])):
                    t_sum-=pixels[row+1][col+1]
                if row+1 in range(len(pixels)) and col-1 in range(len(pixels[0])):
                    t_sum-=pixels[row+1][col-1]
                if row-1 in range(len(pixels)) and col-1 in range(len(pixels[0])):
                    t_sum-=pixels[row-1][col-1]
                n_pixels[row][col]= t_sum
                t_sum=0
        for row in range(len(n_pixels)):
            for col in range(len(n_pixels[0])):
                val = max(0,min(255,n_pixels[row][col]))
                n_pixels[row][col] = [val,val,val]

        return RGBImage(n_pixels)





                #if row-1 not in range(len(pixels)):
                    #kernel.remove(0);
                #if row+1 not in range(len(pixels)):
                    #kernel.remove(2)
                #if col+1 not in range(len(pixels[0])):
                    #for r in kernel:
                      #  kernel[r]= kernel[r][0] + kernel[r][1]
               # if col-1 not in range(len(pixels[0])):
                   # for r in kernel:
                   #     kernel[r] = kernel[r][1] + kernel[r][2]
...
class PremiumPlusMultimediaProcessing(PremiumImageProcessing):
    ...
    def reverb(self, audio):
        ...
        return AudioWave([audio.wave[i] if i<4 else max(min(32767,round((audio.wave[i])+ (audio.wave[i-4]*0.2)+ (audio.wave[i-3]*0.4)+ \
        (audio.wave[i-2]*0.6)+ (audio.wave[i-1]*0.8))),-32768) for i in range(len(audio.wave))])
    ...
    def clip_song(self, audio, start, end):
        ...
        if end<=start:
            return []
        length_of_wave = len(audio.wave)
        start_idx = int(length_of_wave*(start/100))
        end_idx = int(length_of_wave- (length_of_wave*((100-end)/100)))
...
class ImageKNNClassifier:
    ...
    def distance(self, image1, image2):
        ...
        if not isinstance(image1, RGBImage) or not isinstance(image2, RGBImage):
            raise TypeError
        if len(image1.get_pixels())!= len(image2.get_pixels()):
            raise ValueError()
        if len(image1.get_pixels()[0])!= len(image2.get_pixels()[0]):
            raise ValueError
        flat_im1= [c for a in image1.get_pixels() for b in a for c in b]
        flat_im2= [c for a in image2.get_pixels() for b in a for c in b]
        square_diff = list(map(lambda x,y: (x-y)**2, flat_im1, flat_im2))
        return sum(square_diff)**0.5
    ...
    def predict(self, image):
        ...
        dist[0::self.k_neighbors]
        return self.vote(dist)[1]
